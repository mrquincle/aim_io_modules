...
class WriteToFileModule {
private:
  Network yarp;
  std::string module_id;
  
  // private storage for portInp1Value
  int portInp1Value;
  // the port portInp1 itself
  BufferedPort<Bottle> *portInp1;
  
  // private storage for portInp2Value
  int portInp2Value;
  // the port portInp2 itself
  BufferedPort<Bottle> *portInp2;
...
public:
  ...
  WriteToFileModule() {
    cliParam = new Param();
    portInp1 = new BufferedPort<Bottle>();
    portInp2 = new BufferedPort<Bottle>();
  }
  ...
  void Init(std::string module_id) {
    ...
    
    {
      ...
      portName << "/writetofilemodule" << module_id << "/inp1";
      portInp1->open(portName.str().c_str());
    }
    {
      std::stringstream portName; portName.str(); portName.clear();
      portName << "/writetofilemodule" << module_id << "/inp2";
      portInp2->open(portName.str().c_str());
    }
  }
...
protected:
  ...
  inline int *readInp1() {
    Bottle *b = portInp1->read();
    portInp1Value = b->get(0).asInt();
    return &portInp1Value;
  }
  
  inline int *readInp2() {
    Bottle *b = portInp2->read();
    portInp2Value = b->get(0).asInt();
    return &portInp2Value;
  }
